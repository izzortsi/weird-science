...
\noindent
i) $\mathfrak{e}(\mathcal{S}^+) = \{\mathcal{S}_e \ | \ e \in \Gamma_{\mathfrak{P}}\}$, i.e., the elements of $\mathcal{S}^+$ are precisely the subsystems of $\mathcal{S}$ associated with each element;
...
\[\mathcal{H} = (\mathcal{L}, \preceq, \vee, \check{\vee}, \ell)\]
where:

\noindent
i) $\mathcal{L}$ is a collection of systems closed under the operations $\vee$ (superlevel) and $\check{\vee}$ (sublevel);
...
This minimal definition preserves the essential hierarchical structure while remaining consonant with the nested systems framework and the multirelation approach of Lin \cite{LinMa1987}, without imposing well-foundedness or other restrictions that would preclude infinite-level structures. It is, in a sense, the least commitment necessary to formalize the notion that systems are organized into levels, that there exist operations taking one between levels, and that these operations respect a notion of relative position within the hierarchy.

\subsection{Non-well-foundedness and infinite descent}

The framework we have constructed permits, but does not require, hierarchies to extend infinitely in both directions. A hierarchy $\mathcal{H} = (\mathcal{L}, \preceq, \vee, \check{\vee}, \ell)$ is said to be \textit{non-well-founded} if there exists an infinite descending chain
