\documentclass{itt-lean}

\aluno{Igor Strozzi}
\matricula{2025123456}
\professor{Francesco Noseda}
\semestre{2025-2}
\lista{S}
...
\begin{center}
	\Large\textbf{Mereotopological Ontologies with Compositional Algebras:\\
...
This document develops the mathematical foundations for formalizing \emph{mereotopological ontologies} equipped with \emph{compositional algebras} that function as deductive systems. The key insight is that Heyting algebras---the algebraic semantics for intuitionistic logic---provide the natural framework for such compositional structures.
...
The parthood relation $\sqsubseteq$ captures mereological structure:
\begin{itemize}
...
Recall from Chapter 2 (Definition 2.4.2) that a Heyting algebra provides algebraic semantics for intuitionistic logic. We now show why this is the natural choice for a compositional algebra acting on an ontology.

\begin{definicao}[Heyting Algebra]
	A \emph{Heyting algebra} is a bounded distributive lattice $\mathcal{H} = (H, \sqcup, \sqcap, \Rightarrow, 0, 1)$ such that for all $a, b \in H$, the \emph{relative pseudo-complement} $a \Rightarrow b$ exists, characterized by:
	$$
	c \sqcap a \leq b \iff c \leq a \Rightarrow b
	$$
	This is the \emph{adjunction property} (Lemma 2.4.4).
\end{definicao}

\begin{teorema}[Heyting Algebras Model Intuitionistic Logic]
	The completeness theorem (Theorem 2.4.7) states:
	$$
...
\section{The Ontological Algebra: Action of $\mathcal{A}$ on $\mathcal{O}$}
...
The interpretation function $\sem{\cdot}$ maps ontological concepts to algebraic elements, allowing us to reason about conceptual relationships using the Heyting algebra structure.
...
Recall from Proposition 2.4.8 that open sets of a topological space form a Heyting algebra. This provides a canonical model:
...
This connects our ontological algebra to the topological semantics of intuitionistic logic:
\begin{itemize}
	\item Concepts are interpreted as \emph{open sets} (regions of possibility)
	\item Conceptual implication is \emph{interior of material implication}
	\item The excluded middle fails: $A \cup \sim A \neq \mathcal{T}$ in general
\end{itemize}

\begin{exemplo}[Peirce's Law Failure]
	From Example 2.4.9, consider $\mathcal{H} = \mathcal{O}(\mathbb{R})$ with $v(p) = \mathbb{R} \setminus \{0\}$ and $v(q) = \emptyset$. Then:
	$$
	...
	Ontologically: Peirce's law fails because we cannot constructively obtain $p$ merely from knowing that ``if $p$ implied $q$, then $p$ would hold.''
\end{exemplo}

\section{The Three-Element Example}

Consider the simplest non-trivial Heyting algebra: the three-element chain $\mathbf{3} = \{0 < a < 1\}$.
...
\begin{observacao}
	Note that $\sim a = a \Rightarrow 0 = 0$, but $\sim\sim a = 0 \Rightarrow 0 = 1 \neq a$. This demonstrates the failure of double negation elimination in intuitionistic logic (cf. Example 2.1.4(iii)).
\end{observacao}

\subsection{A Simple Ontology}

Define a three-concept ontology:
$$
\mathcal{O}_{\text{simple}} = \{\mathsf{nothing}, \mathsf{entity}, \mathsf{everything}\}
$$
with $\mathsf{nothing} \sqsubseteq \mathsf{entity} \sqsubseteq \mathsf{everything}$.

The interpretation $\sem{\cdot} : \mathcal{O}_{\text{simple}} \to \mathbf{3}$:
\begin{align*}
	\sem{\mathsf{nothing}} &= 0 \\
	\sem{\mathsf{entity}} &= a \\
	\sem{\mathsf{everything}} &= 1
\end{align*}

Then:
\begin{align*}
	\mathsf{entity} \triangleright \mathsf{everything} &= a \sqcap 1 = a \\
	\mathsf{entity} \multimap \mathsf{everything} &= a \Rightarrow 1 = 1 \\
	\mathsf{everything} \multimap \mathsf{entity} &= 1 \Rightarrow a = a
\end{align*}

\section{Lean 4 Formalization}

The complete formalization is self-contained, building all structures from first principles without Mathlib dependencies.
...
The connection between mereotopological ontology and Heyting algebras illuminates a deep correspondence:
...
\begin{enumerate}
	...
	\item \textbf{Compositional Semantics:} The adjunction $a \sqcap b \leq c \Leftrightarrow a \leq b \Rightarrow c$ captures the essence of compositional reasoning: combining $a$ with $b$ to get $c$ is equivalent to having $a$ entail that $b$ leads to $c$.
\end{enumerate}

\begin{observacao}[BHK Interpretation]
	Under the BHK interpretation (Section 2.1), a construction of $\varphi_1 \to \varphi_2$ is a method transforming constructions of $\varphi_1$ into constructions of $\varphi_2$. In our ontological setting:
	$$
