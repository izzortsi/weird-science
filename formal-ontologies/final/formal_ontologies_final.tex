...
\begin{definition}[System (I)]
	Let $X$ be a non-empty set. Let $R^{<i>}_j$ be an $i-$ary relation over $X$. A \textit{system} is a tuple $\mathcal{S} = (X, R)$, where $R = (\{R^{<i>}_j\}_{j\in \mathbb{N}})_{i\in \mathbb{N}}$
%	 
