...
See, if a single relation with a single element suffices, as long as it impacts the system behavior as a whole, to be considered in the definition of a system (as a complex of interacting elements), it is only natural to consider that a system would require in its definition possibly many relations, with, why not, many different arities. It would appeal to reason, then, to consider:

\begin{definition}[System (I)]
	Let $X$ be a set. Let $R^{<i>}_j$ be an $i-$ary relation over $X$. A \textit{system} is a tuple $\mathcal{S} = (X, R)$, where $R = (\{R^{<i>}_j\}_{j\in \mathbb{N}})_{i\in \mathbb{N}}$
	 
	...is a tuple $S = (\domain, \mathbf{R})$ where:
	\begin{itemize}
...
An \emph{extensional relational structure} \citep[cf.][]{genesereth1987} is a tuple $S = (\domain, \mathbf{R})$ where:
\begin{itemize}
    \item $\domain$ is a non-empty set called the \emph{universe of discourse}
    \item $\mathbf{R} = \{R_i\}_{i \in I}$ is a family of relations on $\domain$, where each $R_i \subseteq \domain^{n_i}$ for some arity $n_i \in \mathbb{N}$
\end{itemize}
...
Given a conceptualization $\concept = (\domain, \Sigma, \intrel)$, the set of \emph{intended models} $I(\concept)$ consists of all extensional relational structures $S = (\domain, \mathbf{R})$ satisfying \citep[following][]{guarino2009}:
\begin{enumerate}
