...
Ontology, in the philosophical sense, concerns the most general questions about what there is and how entities are to be counted as such \citep{quine1948, vaninwagen1993, lowe2002survey}. In practice, however, one typically works with \emph{domain} ontologies: structured representations of a circumscribed portion of reality, tailored to a modeling goal.

In the setting of knowledge representation, a formal ontology is commonly understood as an explicit specification of a conceptualization, separating (i) an intensional account of meaning (conceptual relations varying over possible states) from (ii) a formal theory constraining extensional models \citep{gruber1993, gruber1995, uschold1996, studer1998, guarino1995clarification, guarino1998, guarino2009}. This perspective highlights the methodological gap between intended meaning and its logical capture: a theory constrains models, but it does not exhaust the conceptual structure it aims to represent.

Our contribution is to isolate a minimal algebraic substrate for \emph{controlled ontological expansion}. We first make explicit an arity-stratified space of typed relations $E=(E_n)_{n\in\mathbb Z_+}$ (Definition~\ref{def:graded-rel-space}) and a finitary selection mechanism via multi-sections, which supports a precise notion of phenomenal cross-section (partial observation). We then introduce an \emph{ontological algebra}, consisting of an ordered ontology $(O,\le)$, a Heyting algebra $A$, and a monotone interpretation map $\sem{\cdot}:O\to A$ satisfying
$$
c_1\le c_2 \ \Longrightarrow\ \sem{c_1}\le \sem{c_2}.
$$
This provides a compositional semantics in which expansion can be treated as the adjunction of new ontology elements whose interpretations are algebraically constrained refinements of existing ones.
...
A \textit{system} is a pair $\mathcal{S} = (X, R)$ where $X$ is a non-empty set and
$$
R = \big((R^{\langle i \rangle}_j)_{j\in J_i}\big)_{i\in \mathbb{Z}_+}
$$
for some index sets $J_i$, where $R^{\langle i \rangle}_j$ denotes an $i$-ary relation on $X$.
\end{definition}
...
E_n
=
\Big\{\big((D_1,\dots,D_n),R\big)\ \Big|\ D_i \subseteq \domain\ \text{for } i=1,\dots,n,\ \ R \subseteq D_1 \times \cdots \times D_n\Big\}.
$$
For $e=\big((D_1,\dots,D_n),R\big)\in E_n$, we write $\mathrm{ext}(e)=R$ for its extensional component.
\end{definition}
...
A \emph{multi-section} over $\domain$ is a family $s = (s_n)_{n\in \mathbb{Z}_+}$ where, for each $n$, $s_n = (s^{\langle n \rangle}_j)_{j\in J_n}$ is a finite family of elements of $E_n$ (indexed by some finite set $J_n$), and such that the support
$$
\supp(s) := \{n \in \mathbb{Z}_+ \mid J_n \neq \emptyset\}
$$
is finite.
The \emph{extent} of $s$ is the family of extensional relations
$$
|s| := \big(\mathrm{ext}(s^{\langle n \rangle}_j)\big)_{n\in \mathbb{Z}_+,\, j\in J_n}.
$$
\end{definition}
...
The extent $|s|$ forgets the domain-profile $(D_1,\dots,D_n)$ attached to each element of $E_n$, retaining only its extensional component. This operation is useful when passing to the classical notion of extensional relational structure, while the graded family $s$ retains the arity-stratified organization of the selected relations.
...
\label{def:system}
A \emph{system} over $\domain$ is a multi-section over $\domain$.

We write $\mathcal{S} = (\domain, S)$ when emphasizing the domain, and identify $\mathcal{S}$ with the extensional relational structure $(\domain, |S|)$ when passing to extensional presentation.
\end{definition}

\begin{remark}[Relation to the Preliminary Definition]
Definition~\ref{def:system-prelim} presents the relations as an arity-stratified family $\big((R^{\langle i \rangle}_j)_{j\in J_i}\big)_{i\in \mathbb{Z}_+}$. A multi-section makes the same grading explicit while allowing us to treat the selection at each arity uniformly.
\end{remark}
...
\label{def:world}
Let $\domain$ be a domain. A \emph{world} over $\domain$ is a multi-section over $\domain$.
\end{definition}
...
Let $S=(S_n)_{n\in \mathbb{Z}_+}$ be a system over $\domain$, where $S_n=(S^{\langle n\rangle}_j)_{j\in J_n}$. A \emph{phenomenal cross-section} of $S$ is a multi-section $\sigma=(\sigma_n)_{n\in \mathbb{Z}_+}$ such that, for each $n$, there exists a subset $K_n\subseteq J_n$ with
$$
\sigma_n = (S^{\langle n\rangle}_j)_{j\in K_n}.
$$
...
The gap between $S$ and $\sigma$ corresponds to the relations of $S$ whose indices were not selected by $\sigma$: if $\sigma_n=(S^{\langle n\rangle}_j)_{j\in K_n}$, then the ``invisible'' $n$-ary relations are those indexed by $J_n\setminus K_n$. This gap is not a failure of the ontology but rather a constitutive feature of any finite specification before the richness of the real.

\begin{proposition}[Lattice Structure]
\label{prop:phen-lattice}
Let $S_n=(S^{\langle n\rangle}_j)_{j\in J_n}$. Then $(\Phen(S), \sqsubseteq)$ is a bounded distributive lattice. Moreover,
$$
\Phen(S)\cong \prod_{n\in \mathbb{Z}_+} \powerset(J_n),
$$
with order and operations induced pointwise by inclusion, intersection, and union. Explicitly, if $\sigma,\tau\in \Phen(S)$ correspond to families $(K_n)$ and $(L_n)$ with $\sigma_n=(S^{\langle n\rangle}_j)_{j\in K_n}$ and $\tau_n=(S^{\langle n\rangle}_j)_{j\in L_n}$, then:
\begin{itemize}
    \item $\bot$ corresponds to $(\emptyset)_{n\in \mathbb{Z}_+}$ \hfill (empty observation)
    \item $\top=S$ corresponds to $(J_n)_{n\in \mathbb{Z}_+}$ \hfill (complete observation)
    \item $(\sigma \wedge \tau)_n = (S^{\langle n\rangle}_j)_{j\in K_n \cap L_n}$ \hfill (meet)
    \item $(\sigma \vee \tau)_n = (S^{\langle n\rangle}_j)_{j\in K_n \cup L_n}$ \hfill (join)
\end{itemize}
\end{proposition}

\begin{proof}
Since $S$ has finite support, only finitely many $J_n$ are non-empty. A phenomenal cross-section is determined by choosing, for each $n$, a subset $K_n\subseteq J_n$ of the $n$-ary relation-indices of $S$. The lattice operations are inherited pointwise from the Boolean algebra structure on each $\powerset(J_n)$.
\end{proof}
...
Every multi-section $s=(s_n)_{n\in \mathbb{Z}_+}$ over $\domain$, with $s_n=(s^{\langle n\rangle}_j)_{j\in J_n}$, determines an extensional relational structure by taking extensional components:
$$
(\domain, |s|) = \Big(\domain,\big(\mathrm{ext}(s^{\langle n\rangle}_j)\big)_{n\in \mathbb{Z}_+,\, j\in J_n}\Big).
$$
Conversely, given an extensional relational structure $(\domain,(R_i)_{i\in I})$ with $I$ finite, one obtains a multi-section by grouping relations by arity and viewing each $R_i\subseteq \domain^{n_i}$ as an element of $E_{n_i}$ via
$$
\big((\domain,\dots,\domain),R_i\big)\in E_{n_i}.
$$
In this sense, a multi-section carries strictly more information: it can retain arity-stratification and argument-domain profiles as part of the structure, rather than treating them as derived annotations.
\end{remark}
...
The extensional content at a particular world $w$ is recovered as $\mathrm{ext}(\rho^n(w))$---the $n$-ary relation that ``obtains" for that conceptual relation in world $w$.
...
Given a conceptualization $\concept = (\domain, \Sigma, \intrel)$ over a domain space $\langle \domain, W \rangle$, the set of \emph{intended models} $I(\concept)$ consists of all extensional relational structures $\mathcal{S} = (\domain, (R_i)_{i\in I})$ satisfying \citep[following][]{guarino2009}:
\begin{enumerate}
    \item The domain of $\mathcal{S}$ coincides with that of $\concept$
    \item There exists a world $w \in W$ such that for each $\sigma \in \Sigma$, there exists $i\in I$ with $R_i = \mathrm{ext}(\rho_\sigma(w))$
\end{enumerate}
\end{definition}
...
\begin{enumerate}
    ...
    \item There exists a world $w$ such that for each predicate symbol $p \in \vocab$: $I(p) = \mathrm{ext}(\interp(p)(w))$
\end{enumerate}
...
\text{Multi-Section } s = (s_n)_{n\in \mathbb{Z}_+} \arrow[d, "\text{instantiate}"] \\
